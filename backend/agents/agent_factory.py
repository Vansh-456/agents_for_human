...
def create_allocation_agent(tools=None):
    ...
    tools = tools or [propose_allocation_plan]
    return Agent(
...
def create_safety_agent(tools=None):
    if MOCK_AWS: return None
        
    tools = tools or [validate_delivery]
    return Agent(
        name="Safety Agent",
...
def create_signal_agent(tools=None):
    if MOCK_AWS: return None
        
    tools = tools or [create_surplus_event]
    return Agent(
        name="Signal Agent",
...
def create_forecast_agent(tools=None):
    if MOCK_AWS: return None
...
def create_demand_agent(tools=None):
    if MOCK_AWS: return None
        
    tools = tools or [query_ngo_capacity]
    return Agent(
        name="Demand Agent",
...
def create_logistics_agent(tools=None):
    if MOCK_AWS: return None
        
    tools = tools or [find_available_riders, calculate_eta_mins]
    return Agent(
...
def create_trust_agent(tools=None):
    if MOCK_AWS: return None
...
def create_supervisor_agent(tools=None):
    if MOCK_AWS: return None
